...
library(ggplot2)
df2 <- data.frame(sigma = c(rep(c("mom, sigma^2 = 0.1", "mom, sigma^2 = 0.01", "mom, sigma^2 = 0.001"), each=3),
...
ggplot(df2, aes(x=log.n, y=negative.log.mse, group=sigma)) +
...
plot.cover.df <- data.frame(sigma = c(rep("mle",3),rep("mle_boot",3),rep("moom_boot",3)),
                  cover = c(res.mle.cover.s2.0.1,
                            res.mle_boot.cover.s2.0.1,
                            res.mom_boot.cover.s2.0.1),
                  log.n=rep(log(c(100, 1000, 10000)), 3))
plot.cover <- ggplot(plot.cover.df, aes(x=log.n, y=cover, group=sigma)) +
  geom_line(aes(color=sigma))+
...
plot.meanLength.df <- data.frame(sigma = c(rep(c("mle, sigma^2 = 0.1", "mle, sigma^2 = 0.01", "mle, sigma^2 = 0.001"), each=3),
                          ...
                          meanLength = c(res.mle.meanLength.s2.0.1,
...
plot.meanLength <- ggplot(plot.meanLength.df, aes(x=log.n, y=meanLength, group=sigma)) +
  geom_line(aes(color=sigma))+
