...
It is Gamma distribution with parameters $\frac{n}{2}$ and $\lambda = \frac{n}{2\sigma^2}$.

rescaled to $ \frac{n\sigma^2}{\sigma^2}  \sim Gamma(\frac{n}{2},\frac{1}{2})$

By Theorem B.9.4,  $ \frac{n\sigma^2}{\sigma^2}  \sim Gamma(\frac{n}{2},\frac{1}{2})$ is pivotal. 
...
$= P(\frac{\sum_{i=1}^{n} X^2}{q_{0.025}}\geq \frac{1}{\sigma^2} \geq \frac{\sum_{i=1}^{n} X^2{q_{0.975}})$ 
...
We found that for MLE 95% confidence interval is $(\frac{\sum_{i=1}^{n} X^2}{q_{0.975}}, \frac{\sum_{i=1}^{n} X^2}{2q_{0.025}})$ where $q_{0.975}$ and $q_{0.025}$ come from the Gamma distribution with $r = n/2$ and $\lambda = \frac{1}{2}$.
```{R}
...
X^2 <- mean(x)
mle_lower[i] <- \sum_{i=1}^{n} X^2/(q_0.975)
# based on exact MLE CI (part d)
mle_upper[i] <- \sum_{i=1}^{n} X^2/(q_0.025)
